...
- Runtime package excludes template sources by default: `agent-settings/templates/` and legacy `references/` (see `verify/package_manifest.yaml` field `excludes_references`)
...
- Template source-of-truth: `agent-settings/templates/` (canonical). Transition: if `agent-settings/templates/` is missing, MAY read templates read-only from legacy `references/` only to populate content; rendered outputs MUST still be written under `agent-settings/roles/<role-slug>/` and validated there.
- Outputs:
...
- `agent-settings/templates/` — canonical template sources (migrated from legacy `references/`; same heading contracts as former `goclaw-template-*.md` files).
...
- `verify/package_manifest.yaml` minimum schema:
  - `generation_id`, `generated_at`, `commit_sha` (same tuple as generation window)
  - `excludes_references` (boolean, MUST be `true` for shipped runtime bundles): when true, runtime bundle MUST exclude template sources (`agent-settings/templates/` and legacy `references/` if present)
  ...
  - Optional `layout_root_files_optional`: MAY list `README.md` only when present; MUST NOT list `references/` or `agent-settings/templates/` as required runtime paths
...
Runtime packaging note: shipped bundles exclude template sources (`agent-settings/templates/` and legacy `references/`) by default per `excludes_references: true`; the canonical on-disk layout applies before packaging.
...
1. Read template from `agent-settings/templates/` (or transitional read-only legacy `references/` only if templates directory missing).
...
- Recovery: create canonical `agent-settings/` tree; migrate templates from legacy `references/` if needed; re-render roles; rerun verification pipeline from `TEAM_RESEARCH` forward.
